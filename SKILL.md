...
**Signal thresholds** (use consistently across all modes — these are the source of truth for the rules engine):
...
- Promoter pledge: >10% → flag always
- Health verdict mapping:
  - D/E < 0.5 AND interest coverage > 4 AND current ratio > 1.5 → Strong
  - D/E 0.5–1 OR coverage 2–4 OR current ratio 1–1.5 → Adequate
  - D/E 1–2 OR coverage 1.5–2 → Stretched
  - D/E > 2 OR coverage < 1.5 → Distressed
- Returns verdict mapping:
  - ROE > 20% AND ROCE > 15% → Excellent
  - ROE > 15% OR ROCE > 12% → Good
  - ROE 10–15% AND ROCE 8–12% → Average
  - ROE < 10% OR ROCE < 8% → Poor
- Growth verdict mapping (based on 3Y revenue CAGR):
  - > 20% → Strong
  - 10–20% → Moderate
  - 5–10% → Weak
  - < 5% or negative → Declining
- Ownership verdict mapping:
  - Promoter ≥ 50% AND pledge = 0 → High Conviction
  - Promoter ≥ 30% AND pledge < 10% → Mixed
  - Pledge > 10% OR promoter declining trend → Concerning
...
Fill all 8 tabs: Snapshot · Valuation · Growth · Health · Returns · Peers · Ownership · The View.

- Flag any missing metric inline as 🚩 Data unavailable — verify at [source].
- The **View** tab carries the summary: 3 strengths, 2–3 watch-points, 1 thing to track, overall quality, and the disclaimer.
- Add a one-line **data confidence** note (High / Medium / Low) based on how many metrics came from live sources vs unavailable.
- Never fabricate numbers. Never predict. No targets.
